...
export async function exportPioneersToPdf(
  pioneersData: PioneerPdfData[],
): Promise<void> {
  await loadScript(
  ...
  doc.text(
    `Theiarco  •  ${dateStr}  •  ${pioneersData.length} pioneer${pioneersData.length !== 1 ? "s" : ""}`,
    14,
    22,
  );
  ...
  const head = [
    [
      ...
      "Avg Hrs",
      "Status",
      ...SERVICE_YEAR_MONTHS.map((m) => m.substring(0, 3)),
    ],
  ];
