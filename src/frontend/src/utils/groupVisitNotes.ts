...
export function parseGroupVisitNotes(
  combinedNotes: string,
): ParsedGroupVisitNotes {
  ...
  const discussionMatch = combinedNotes.match(
    /Discussion Topics:\s*\n([\s\S]*?)(?=\n\nNotes for Overseer:|\n\nNotes for Assistant:|\n\nNext Planned Visit:|$)/,
  );
  const overseerMatch = combinedNotes.match(
    /Notes for Overseer:\s*\n([\s\S]*?)(?=\n\nNotes for Assistant:|\n\nNext Planned Visit:|$)/,
  );
  const assistantMatch = combinedNotes.match(
    /Notes for Assistant:\s*\n([\s\S]*?)(?=\n\nNext Planned Visit:|$)/,
  );
  const nextVisitMatch = combinedNotes.match(
    /Next Planned Visit:\s*\n([\s\S]*?)$/,
  );
